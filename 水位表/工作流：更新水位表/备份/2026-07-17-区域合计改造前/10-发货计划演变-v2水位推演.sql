...
ALTER TABLE temp_v2_total_stage
    ADD UNIQUE INDEX idx_v2_total_stage_key (asin, country, shop, `date`);
...
-- 只要求覆盖所有可推演分店，以及由这些分店生成的合计；孤立合计不进入 stage。
INSERT INTO temp_v2_projection_guard (guard_value)
SELECT IF(
...
SET @v2_branch_rows = (SELECT COUNT(*) FROM temp_v2_branch_stage);
SET @v2_total_rows = (SELECT COUNT(*) FROM temp_v2_total_stage);
