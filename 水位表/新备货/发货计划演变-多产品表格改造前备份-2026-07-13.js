...
function ProductSwitcher({
  ...
  h('span', { style: { fontSize: 13.5, color: '#475467' } }, '型号'),
  h(Select, {
    ...commonSelectProps,
    value: selectedModel,
  ...
  h('span', { style: { fontSize: 13.5, color: '#475467' } }, 'ASIN / 站点'),
  h(Select, {
    ...commonSelectProps,
  ...
  h(Tag, { bordered: false, style: { marginInlineEnd: 0, fontSize: 13.5 } }, `${productCount} 个可用商品`));
}
      ...
      h('table', { style: { width: '100%', minWidth: w12Open ? 1680 : 1560, borderCollapse: 'separate', borderSpacing: 0, background: '#fff', fontVariantNumeric: 'tabular-nums' } },
        h('thead', null,
          h('tr', null,
            ...
            h('th', { colSpan: shownWeeks.length, style: { padding: 9, background: '#fff4df', color: '#805000', borderBottom: '1px solid #edd8aa', fontSize: 14 } },
        ...
        h('tbody', null,
          ...
          expanded ? h('tr', null, h('td', { colSpan: 9 + shownWeeks.length + 1, style: { padding: 14, background: '#fbfcfe' } },
            h('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 } },
              h('b', { style: { fontSize: 14 } }, '到货与安全库存天数趋势'),
...
function ShipmentEvolutionBlock() {
  ...
  const [selectedSale, setSelectedSale] = useState(IS_ADMIN ? '' : CURRENT_USERNAME);
    ...
    Promise.all([resolveParams(), requestEligibleProducts()])
      .then(([initialParams, productRows]) => {
        ...
        if (firstProduct) {
          ...
          if (IS_ADMIN) setSelectedSale(firstProduct.sale_owner || '');
  ...
  useEffect(() => {
    ...
    const unsubscribers = [ctx.router, ctx.app?.router?.router].filter((router) => typeof router?.subscribe === 'function')
      .map((router) => router.subscribe(() => {
        ...
        if (IS_ADMIN && allowedProduct.sale_owner !== selectedSale) {
          setSelectedSale(allowedProduct.sale_owner || '');
  ...
  }, [catalogReady, products, selectedSale]);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const nextLevelName = await requestModelLevel(activeParams, waterData?.model || dailyData[0]?.model || totalData[0]?.model);
  ...
  const productOptions = useMemo(() => visibleProducts.map((row) => ({
    ...
    label: `${row.model || '未填型号'} · ${row.asin} · ${row.country} · ${row.status}`,
  })), [visibleProducts]);
  ...
  function changeSale(sale) {
    ...
    setSelectedModel(ALL_MODELS);
    const candidates = products.filter((row) => row.sale_owner === sale);
  ...
  h('div', { style: { display: 'flex', alignItems: 'center', gap: 9, flexWrap: 'wrap', marginBottom: 10 } },
    ...
    h('span', { style: { marginLeft: 'auto', fontSize: 13, color: C.muted } }, `${params.asin || '-'} · ${params.country || '-'} · ${params.shop || TOTAL_SHOP}`)),
  ...
  h('div', { style: { background: C.panel, border: '1px solid #dfe4eb', borderRadius: 9, padding: 12 } },
    ...
    h('div', { style: { display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', marginBottom: 9 } },
      ...
      h('span', { style: { marginLeft: 'auto', fontSize: 13, color: C.muted } }, hiddenByWarning ? '0 条' : currentRow ? '1 条' : '0 条'),
