...
async function run() {
  ...
  function App() {
    const [rows, setRows] = useState([]), [region, setRegion] = useState('美国/欧洲'), [loading, setLoading] = useState(true), [editingId, setEditingId] = useState(null), [draft, setDraft] = useState({});
    async function reload() { setLoading(true); try { setRows(await readRows()); } catch (e) { message.error('读取失败:' + (e?.message || e)); } setLoading(false); }
    ...
    return React.createElement(ConfigProvider, null, React.createElement('div', { className: 'aok-cfg-wrap', style: { fontFamily: "'PingFang SC','Microsoft YaHei',-apple-system,sans-serif", color: C.text } },
      ...
      React.createElement('div', { style: { fontSize: 12, color: C.muted, lineHeight: 1.7, margin: '2px 0 12px' } }, '按三个区域(美国/欧洲、加拿大、日本)分别配置活动日期与初始系数，支持手动添加。', React.createElement('span', { style: { color: '#a1a1aa' } }, '采运部维护 · 改动将生效于后续联动板块')),
      React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 } }, React.createElement('span', { style: { fontSize: 12, color: C.muted } }, '当前区域'), React.createElement(Select, { size: 'small', value: region, style: { width: 150 }, options: ['美国/欧洲', '加拿大', '日本'].map(v => ({ value: v, label: v })), onChange: setRegion }), React.createElement(Button, { size: 'small', type: 'primary', style: { marginLeft: 'auto' }, onClick: () => { const row = { id: '__new', region, day_type: '', category: '基础类型', start_date: '', end_date: '', coef: 1 }; setEditingId('__new'); setDraft(row); } }, '+ 添加')),
