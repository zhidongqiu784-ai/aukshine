...
async function requestDailySales(params, forceTotal = false) {
  ...
  const response = await apiRequest({
    url: 'daily_sales:list', method: 'get', params: {
      ...
      fields: [
        'id', 'asin', 'country', 'shop', 'model', 'date', 'type', 'weighted_sales', 'maybe_sales',
...
function buildWeeks(shipments) {
  ...
  const defaultTransit = Math.max(0, ...shipments.map((row) => numberValue(row.warehouse_days)));
  return Array.from({ length: 7 }, (_, index) => {
    ...
    const transit = Math.max(defaultTransit, ...rows.map((row) => numberValue(row.warehouse_days)));
    const coverStart = addDays(start, transit);
    return {
      key: `W${index + 1}`, index, start, end, rows, quantity,
      coverStart, coverEnd: addDays(coverStart, 6),
...
function ratioInfo(row) {
  const ratio = numberValue(row?.days_for_sale) / 30;
...
function EvolutionTable({ params, currentRow, totalRow, dailyRows, weeks }) {
  const [expanded, setExpanded] = useState(false);
  const [w12Open, setW12Open] = useState(false);
  const [drawerWeek, setDrawerWeek] = useState(null);
  const ratio = ratioInfo(currentRow);
  ...
  const orderQty = weeks[5].quantity + weeks[6].quantity;
  const undelivered = numberValue(totalRow?.quantity_receive);
  const net = Math.max(0, orderQty - undelivered);
  const productCells = [
    params.country || '-', '-', currentRow?.model || '-', params.asin || '-', fmt(currentRow?.weighted_sales, 1),
    currentRow?.type || '-', `${fmt(ratio.value, 1)} ${ratio.name}`, currentRow?.type || '未标注',
  ];
      ...
      h('table', { style: { width: '100%', minWidth: w12Open ? 1540 : 1430, borderCollapse: 'separate', borderSpacing: 0, background: '#fff' } },
        ...
        h('tbody', null,
          h('tr', null,
            ...
            h('div', { style: { fontSize: 10, color: '#7d6a43' } }, `计划 ${fmt(orderQty)} − 未交货 ${fmt(undelivered)}`),
            h('div', { style: { fontSize: 9.5, color: '#9a8150', marginTop: 2 } }, '未交货固定取合计行'))),
...
function ShipmentEvolutionBlock() {
  ...
  useEffect(() => {
    let active = true;
    resolveParams().then((next) => { if (active) setParams({ ...next, shop: next.shop || TOTAL_SHOP }); });
    return () => { active = false; };
  }, []);
  ...
  useEffect(() => {
    const unsubscribers = [ctx.router, ctx.app?.router?.router].filter((router) => typeof router?.subscribe === 'function')
      .map((router) => router.subscribe(() => {
        ...
        setParams((current) => ({ asin: next.asin || current.asin, country: next.country || current.country, shop: next.shop || current.shop || TOTAL_SHOP }));
      }));
  ...
  const loadData = useCallback(async () => {
    if (!params.asin || !params.country) {
      ...
      setError('缺少页面参数 asin 或 country，无法加载发货计划。'); setLoading(false); return;
    }
    const activeParams = { ...params, shop: params.shop || TOTAL_SHOP };
    setLoading(true); setError('');
    try {
      const needsSeparateTotal = activeParams.shop !== TOTAL_SHOP;
      const [shopList, dailyData, totalData, shipmentData] = await Promise.all([
  ...
  h('div', { style: { background: C.panel, border: '1px solid #dfe4eb', borderRadius: 9, padding: 12 } },
    h(ShopSwitcher, { params, shops, loading, onChange: changeShop, onRefresh: () => setRefreshSeed((value) => value + 1) }),
    ...
    h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(320px,1fr))', gap: 8, marginBottom: 8 } },
      ...
      h(InfoBox, null, h('b', null, '② 看覆盖前沿　'), '覆盖售卖期按周起点与计划中的物流天数推算；展开行查看安全天数趋势。')),
    ...
    h('div', { style: { display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', marginBottom: 9 } },
      ...
      h(Select, { size: 'small', disabled: true, value: '全部销售', options: [{ value: '全部销售', label: '全部销售' }], style: { width: 110 } }),
      h(Select, { size: 'small', disabled: true, value: '全部产品标签', options: [{ value: '全部产品标签', label: '全部产品标签' }], style: { width: 135 } }),
    ...
    h(Legend),
    loading ? h('div', { style: { minHeight: 360, display: 'flex', alignItems: 'center', justifyContent: 'center' } },
      h(Spin, { size: 'large', tip: '正在读取 daily_sales 与 simulate_shipment...' }))
      : hiddenByWarning ? h(Empty, { description: '当前记录不属于预警范围' })
        : currentRow ? h(EvolutionTable, { params, currentRow, totalRow, dailyRows, weeks })
          : h(Empty, { description: `当天没有 ${params.shop || TOTAL_SHOP} 的 daily_sales 数据` }),
