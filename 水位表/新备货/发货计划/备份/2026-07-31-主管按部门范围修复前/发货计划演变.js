...
function nextMonday(value) {
  ...
  date.setHours(0, 0, 0, 0);
  const weekday = date.getDay();
  date.setDate(date.getDate() + (weekday === 0 ? 1 : weekday === 1 ? 0 : 8 - weekday));
  return date;
}
  ...
  return requestProductBatches(products, (batch) => apiRequest({
    url: 'daily_sales:list', method: 'get', params: {
      ...
      fields: [
        ...
        'v2_add', 'v2_inventory', 'v2_days_for_sale', 'v2_days_cover', 'v2_sale_days_cover', 'v2_on_the_way', 'v2_calculated_at',
      ].join(','),
      filter: JSON.stringify({ $and: [
...
function V19TrendChart({ row, changes, role, poApproved, orderWeek, channelOptions, onApply, onSandbox }) {
  ...
  const existingValues = dates.map((date, index) => {
    ...
    const value = numberValue(dailyMap.get(date)?.v2_sale_days_cover, NaN);
  ...
  const sysValues = dates.map((date, index) => {
    ...
    const value = index <= todayIndex
      ...
      : numberValue(daily?.v2_days_cover, numberValue(daily?.v2_days_for_sale, NaN));
    return Number.isFinite(value) ? value : null;
  });
  ...
  const baseNodes = row.weeks.map((week, index) => {
    ...
    return { index, week, baseQty, channel, baseChannel: channel, baseShift, baseChannelOverride: change?.channel || null, baseDates: v19BatchDates(row, week, change?.channel || null, baseShift) };
  });
  ...
  const buildPreview = (activeMods) => {
    ...
    const invArr = new Array(dates.length).fill(NaN);
    const demArr = new Array(dates.length).fill(0);
    ...
    for (let start = todayIndex; start < dates.length; start += 1) {
      if (values[start] === null || !Number.isFinite(invArr[start])) continue;
      if (!(invArr[start] > 0)) { values[start] = 0; continue; }
      let remaining = invArr[start];
      let days = 0;
      for (let cursor = start; cursor < dates.length && days < 60; cursor += 1) {
        const need = Math.max(0, Number.isFinite(demArr[cursor]) ? demArr[cursor] : 0);
        if (remaining < need) break;
        remaining -= need; days += 1;
      }
      values[start] = Math.min(60, days);
    }
  ...
  const visibleNodes = displayChart.nodes.map((node) => {
  ...
  const renderBatchBase = (info, options = {}) => {
    ...
    return [
      ...
      h('div', { key: 'arrival-date', style: rowStyle }, h('span', { style: { color: muted } }, '到货日期　'), h('b', { style: { color: info.arrivalChanged ? changed : color } }, info.arrivalChanged ? `${dateText(info.baseArrival)} → ${dateText(info.arrival)}` : dateText(info.arrival))),
  ...
  function applyAll() {
    ...
    const evidence = `基于 daily_sales.v2_days_cover(真实可撑天数)的拖动预览 ${bundle.length} 处：当前系统曲线超上限 ${baseMetric.over} 天、最低 ${fmt(baseMetric.min, 1)} 天。最终安全区间结果以提交工作流服务端重算为准。`;
  ...
  return h('div', {
    ...
    h('div', {
      ...
      ...laidOutNodes.map((node) => {
        ...
        return h('button', {
          key: `node-${node.index}`, type: 'button', 'aria-label': `W${node.index + 1} · 系统建议 ${fmt(node.qty)} 台${stateText} · 到货 ${shortDate(node.arrival)}`, onPointerDown: (event) => startDrag(event, node),
    ...
    h('div', { style: { margin: '6px 0 0 40px', display: 'flex', gap: 14, flexWrap: 'wrap', fontSize: 11, color: '#5a6169' } },
      h('span', null, h('i', { style: { display: 'inline-block', width: 18, borderTop: '2px solid #5b7cfa', marginRight: 5, verticalAlign: 'middle' } }), '系统预估销量·安全库存天数'),
      ...
      h('tbody', null,
        ...rows.flatMap((row, rowIndex) => {
          ...
          const cells = shownIndices.map((index) => {
            const week = row.weeks[index]; const key = v19ChangeKey(row.key, index); const change = changes[key]; const value = v19WeekValue(row, index, changes); const state = v19WeekState(row, index); const sandbox = sandboxes[row.key]?.[index]; const simulatedSuggest = sandbox?.qty == null ? null : numberValue(sandbox.qty); const displayValue = simulatedSuggest == null ? value : simulatedSuggest; const status = change?.status || 'ok'; const statusStyle = V19_STATUS_STYLE[status]; const newAlgorithm = week.newQty > 0; const actualPresent = state?.kind === 'actual';
            ...
            const interactionHint = sandbox ? sandboxHintOf(row, week, change, value, sandbox, state) : change ? `${V19_CHANGE_MARK[change.type]} ${V19_CHANGE_LABEL[change.type]}：系统建议 ${fmt(change.from)} → ${fmt(change.to)} 台 · ${V19_STATUS_TEXT[status]} · ${change.reason}${stateHint}${baselineHint}` : week.rows.length || week.shippedRows.length || week.actualPlanRows.length ? `点击查看：系统建议 ${week.rows.length} 条${stateHint}${baselineHint}` : `该周无排；点击可提出修改申请${baselineHint}`;
            ...
            return h(Tooltip, { key: index, title: interactionHint + leadHint }, h('td', { onClick: () => onOpenDetail(row, index), style: { position: 'relative', minWidth: 116, padding: '8px 7px', textAlign: 'center', borderBottom: border, borderRight: border, cursor: 'pointer', fontWeight: change || displayValue ? 800 : 500, color: sandbox ? '#5b3fc4' : change ? statusStyle.color : displayValue ? '#1f2329' : '#c2c8d0', background: sandbox ? '#f2edff' : actualPresent ? '#f3fbf6' : index >= 5 ? '#f6f3ff' : index === 0 ? '#ededf0' : statusStyle.bg, boxShadow: sandbox ? 'inset 0 0 0 2px #8b6cf0' : change && status !== 'ok' ? `inset 0 0 0 2px ${statusStyle.border}` : index >= 5 ? 'inset 0 2px 0 #8b6cf0, inset 0 -2px 0 #8b6cf0' : 'none' } }, sandbox ? h('span', { style: { position: 'absolute', top: 1, right: 3, fontSize: 9.5, color: '#6b4fd0', fontWeight: 900 } }, '沙盘') : change ? h('span', { style: { position: 'absolute', top: 2, right: 3, fontSize: 10.5, fontWeight: 900, color: statusStyle.color } }, V19_CHANGE_MARK[change.type]) : newAlgorithm ? h('span', { style: { position: 'absolute', top: 2, right: 3, color: '#7c3aed', fontSize: 10.5 } }, '⟳') : actualPresent ? h('span', { style: { position: 'absolute', top: 2, right: 3, color: '#1a6d49', fontSize: 10.5 } }, '◆') : null, change && status !== 'ok' ? h('span', { style: { position: 'absolute', top: 1, left: 3, fontSize: 10 } }, status === 'rej' ? '⛔' : '⏳') : null, week.eventLead ? h('span', { style: { position: 'absolute', bottom: 1, right: 3, fontSize: 9.5, color: '#1a5fb4', fontWeight: 800 } }, '✈') : null, weekCellBody(value, state, { active: Boolean(sandbox), suggest: simulatedSuggest }, week.baseline)));
          });
  ...
  return h('div', { style: { width: '100%', minWidth: 0, margin: 0, padding: 22, background: '#eef1f5', color: '#1f2329', fontFamily: '-apple-system,"PingFang SC","Microsoft YaHei",sans-serif', fontSize: 13.5, lineHeight: 1.6, boxSizing: 'border-box', WebkitFontSmoothing: 'antialiased', fontVariantNumeric: 'tabular-nums' } },
    ...
    h('div', { style: { background: '#fff', border: '1px solid #e6e9ee', borderRadius: 9, boxShadow: '0 1px 8px rgba(0,0,0,0.05)', padding: '16px 18px' } },
      guideOpen ? h(React.Fragment, null,
        ...
        h('div', { style: { fontSize: 12, color: '#36507e', background: '#eef4ff', borderLeft: '3px solid #3370ff', borderRadius: 5, padding: '9px 12px', lineHeight: 1.7, marginBottom: 12 } }, '数据口径：库存 / 安全天数 / 在途读取 daily_sales.v2_*；实际计划只读取发货日期从当前 W1 起、数量大于 0 的旧 simulate_shipment 记录，不以是否生成货件编号判断；系统建议读取 simulate_shipment.plan_source=shipment_plan_v2。当前 W1 之前的计划视为已经发走并转为在途，不再单独展示。', h('b', { style: { color: '#1f2329' } }, '格子主数和图表节点 = 系统建议量；实际计划单独展示并固定不随模拟变化。拖动和审批只改系统建议，不改实际计划。净额列 = 只读结果 = W6+W7 建议量 − 未交货余量（2.5.2）。'), '净额 = 0 时本批不下新单。周二采购在净额区「生成下单计划」→ 下单执行看板（工厂×规格 · 调拨在看板算）→ 终审。')) : null,
      ...
      catalogLoading || loading ? h('div', { style: { minHeight: 360, display: 'flex', alignItems: 'center', justifyContent: 'center' } }, h(Spin, { size: 'large', tip: '正在读取 daily_sales.v2、水位标签、系统建议、上周基线和当前 W1 起的实际计划...' })) : visibleRows.length ? h(V19Table, { rows: visibleRows, allScopeRows: productViews, changes, confirmedRows, role, orderWeek, poApproved, logisticsLeads, onEdit: openEdit, onOpenDetail: (row, weekIndex) => { setAuditNote(''); setDetail({ row, weekIndex }); } }) : h(Empty, { description: mine ? '当前角色没有待处理商品' : '当前筛选范围没有商品' })),
