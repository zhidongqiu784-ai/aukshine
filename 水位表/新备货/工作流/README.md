...
本目录保存已部署资产。线上已创建四个工作流，但均保持停用、执行次数为 0；未获明确确认前不得启用或执行写入链路。

## 文件

- `01-提交申请-服务端校验与安全闸.sql`：服务端重查用户、角色、商品范围和原计划，执行边界校验，并用拟议计划替换目标批次后逐日重算 v2 水位风险。
- `02-审核流转-服务端校验与写回.sql`：主管、采购、终审权限回查，按 `id + status + row_version` 乐观锁流转并写审计。
- `03-应用计划-乐观锁写回.sql`：再次比对 `simulate_shipment` 原值，重算 `season / warehouse_days / add_date`，应用计划并置为 `APPLYING`。
- `04-应用计划-推演完成.sql`：定向 v2 水位推演成功后置为 `APPLIED / OK` 并写审计。
- `workflows.manifest.json`：四个默认禁用工作流的触发器、节点顺序、分支和配置模板。
- `05-simulate_shipment-v2保护-暂不启用.md`：说明为什么暂不能对 `simulate_shipment` 使用全局拦截。

应用工作流的第二个 SQL 节点复用上级目录的
`发货计划演变-v2_应用后定向水位推演.sql`。部署时必须按 manifest 的
