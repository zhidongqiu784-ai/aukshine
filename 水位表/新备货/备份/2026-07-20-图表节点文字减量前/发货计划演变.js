  ...
  return h('div', {
    ...
    h('div', {
      ...
      h('svg', { viewBox: `0 0 ${W} ${H}`, shapeRendering: 'geometricPrecision', style: { position: 'absolute', inset: 0, width: '100%', height: '100%', display: 'block' } },
        ...
        ...visibleNodes.flatMap((node) => {
          const above = node.y > T + 46; const locked = node.index < 2; const modified = changedKeys.includes(String(node.index)) || (draft?.pending && draft.index === node.index); const hasQty = numberValue(node.totalQty) > 0;
          ...
          return [
            ...
            hasQty ? h('text', { key: `node-qty-${node.index}`, x: node.x, y: node.y + (above ? -30 : 22), fill: modified ? '#5b3fc4' : '#1f2329', fontSize: 12.5, fontWeight: 800, ...textStyle }, `W${node.index + 1} · ${fmt(node.totalQty)} 台${modified ? '*' : ''}`) : null,
            hasQty ? h('text', { key: `node-date-${node.index}`, x: node.x, y: node.y + (above ? -15 : 36), fill: modified ? '#5b3fc4' : '#5a6169', fontSize: 11.5, fontWeight: 600, ...textStyle }, `建议 ${fmt(node.qty)} · 入库 ${shortDate(node.arrival)}${locked ? ' 🔒' : ''}`) : null,
          ];
        })),
      ...visibleNodes.map((node) => {
        return h('button', {
          key: `node-${node.index}`, type: 'button', 'aria-label': `W${node.index + 1} · 合计 ${fmt(node.totalQty)} 台 · 建议 ${fmt(node.qty)} 台 · 到货 ${shortDate(node.arrival)}`, onPointerDown: (event) => startDrag(event, node),
      ...
      hoveredBatch ? h('div', { style: { position: 'absolute', ...(hoveredBatch.x / W > 0.76 ? { right: 12 } : { left: `calc(${xPercent(hoveredBatch.x)} + 12px)` }), ...(hoveredBatch.y / H > 0.52 ? { bottom: `calc(${Math.max(5, 100 - hoveredBatch.y / H * 100)}% + 14px)` } : { top: `calc(${Math.max(5, hoveredBatch.y / H * 100)}% + 14px)` }), zIndex: 6, width: 300, maxWidth: 'calc(100% - 24px)', maxHeight: 'calc(100% - 20px)', overflowY: 'auto', boxSizing: 'border-box', whiteSpace: 'normal', overflowWrap: 'anywhere', wordBreak: 'break-word', padding: '9px 11px', borderRadius: 7, background: 'rgba(31,35,41,0.94)', color: '#fff', fontSize: 12, lineHeight: 1.55, pointerEvents: 'none', boxShadow: '0 6px 22px rgba(0,0,0,.28)' } },
        h('b', null, `W${hoveredBatch.index + 1} 发货周 · ${shortDate(hoveredBatch.week.start)}~${shortDate(hoveredBatch.week.end)}${hoveredBatch.index < 2 ? '（W1–W2 守工厂节奏）' : ''}`),
        h('div', null, `合计 ${fmt(hoveredBatchInfo.totalQty)} 台 = 真实在途 ${fmt(hoveredBatchInfo.actualQty)} + 建议 ${fmt(hoveredBatchInfo.suggestQty)}`),
        h('div', { style: { color: '#ffd479' } }, `真实在途入库：${hoveredBatchInfo.actualRows}（已进蓝线，不可拖）`),
        h('div', null, `建议预计入库 ${dateText(hoveredBatch.arrival)} → 入仓 ${hoveredBatch.warehouseDays || 0} 天 → 可售 ${formatDate(hoveredBatch.sellable)} · 渠道 ${hoveredBatch.mod.channel || hoveredBatch.channel}`),
        h('div', { style: { color: '#cbd6ff' } }, `拖动只改变建议 ${fmt(hoveredBatch.qty)} 台，真实在途不变 · 约 +${fmt(hoveredBatch.qty / fallbackDaily, 1)} 天安全库存`)) : null),
    h('div', { style: { margin: '6px 0 0 40px', display: 'flex', gap: 14, flexWrap: 'wrap', fontSize: 11, color: '#5a6169' } },
      ...
      h('span', null, h('i', { style: { display: 'inline-block', width: 18, borderTop: '2px dashed #c0392b', marginRight: 5, verticalAlign: 'middle' } }), '上下限（站点×淡旺季）'),
        h('span', null, '◌ W节点（合计=真实在途+建议；拖动只改建议）'),
