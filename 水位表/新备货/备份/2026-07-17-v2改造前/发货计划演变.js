...
function ShipmentEvolutionBlockV19() {
  ...
  const loadData = useCallback(async () => {
    if (!catalogReady) return;
    const requestId = ++requestSequence.current;
    if (!scopedProducts.length) { setDailyRows([]); setTotalRows([]); setShipments([]); setWaterRows([]); setModelLevels([]); setShops([]); setLoading(false); return; }
    setLoading(true); setError('');
    try {
      const activeShop = params.shop || TOTAL_SHOP;
      const dailyPromise = requestDailySales(scopedProducts, activeShop);
      const directoryShipmentPromise = requestShipments(shopScopeProducts, TOTAL_SHOP);
      const [shopData, directoryShipments, dailyData, totalData, shipmentData, waterData, levelData] = await Promise.all([
        ...
        requestWaterProducts(scopedProducts), requestModelLevels(scopedProducts),
      ]);
      if (requestId !== requestSequence.current) return;
      ...
      setDailyRows(dailyData); setTotalRows(totalData); setShipments(shipmentData); setWaterRows(waterData); setModelLevels(levelData);
    } catch (requestError) {
      if (requestId === requestSequence.current) { setError(requestError?.message || String(requestError)); setDailyRows([]); setTotalRows([]); setShipments([]); setWaterRows([]); setModelLevels([]); }
