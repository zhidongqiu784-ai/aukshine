...
-- 真实未入库量直接复用 expected_inventory：
-- qty_shipped > 0 的 remaining 为真实在途；qty_shipped = 0 的旧模拟计划不进入 v2。
-- 只合并 plan_source = 'shipment_plan_v2'，保留 shippment_id 去重。
...
WITH real_shipment AS (
    ...
    WHERE ei.asin = @v2_target_asin
      AND ei.country = @v2_target_country
      AND COALESCE(ei.qty_shipped, 0) > 0
