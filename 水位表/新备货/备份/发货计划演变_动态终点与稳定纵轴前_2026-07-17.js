  ...
  return requestProductBatches(products, (batch) => {
    ...
    url: 'simulate_shipment:list', method: 'get', params: {
      ...
      fields: [
        ...
        'number', 'date', 'season', 'warehouse_days', 'add_date', 'arrival_date', 'shippment_id', 'plan_source',
      ].join(','),
      filter: JSON.stringify({ $and: filters }),
    },
    });
...
function V19ScopeBar({ selectedSale, saleOptions, productCount, params, shops, loading, onSaleChange, onShopChange, onRefresh }) {
  ...
  const shopOptions = [
    { value: TOTAL_SHOP, label: '合计（全部店铺）' },
    ...shops.map((shop) => typeof shop === 'string'
      ? { value: shop, label: shop }
      : { value: shop.name, label: `${shop.name}（${shop.productCount} ASIN）` }),
  ];
  return h('div', { style: { background: '#f8fafc', border: '1px solid #dfe4eb', borderRadius: 8, padding: '8px 11px', marginBottom: 10 } },
    h('div', { style: { display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' } },
      ...
      h('span', { style: { marginLeft: 6, fontSize: 12.5, fontWeight: 700, color: '#3a4763' } }, '店铺视角'),
      h(Select, {
        size: 'small', value: params.shop || TOTAL_SHOP, options: shopOptions, onChange: onShopChange,
        showSearch: true, optionFilterProp: 'label', loading, style: { width: 230, fontSize: 12.5 },
      }),
...
function V19TrendChart({ row, changes, role, poApproved, orderWeek, channelOptions, onApply, onSandbox }) {
  ...
  const W = 1600; const H = 285; const L = 54; const R = 22; const T = 28; const B = 42;
  ...
  const buildNodes = (activeMods) => baseNodes.map((base) => {
    ...
    return {
      ...base, mod, qty, shift, channel, ...datesValue, storageDate, pointIndex,
    };
  });
  ...
  const numericSeries = [...sysValues, ...simValues, ...existingValues].filter(Number.isFinite);
  ...
  const transit = numberValue(row.summaryRow?.on_the_way);
  const receive = numberValue(row.waterRow?.quantity_receive ?? row.totalRow?.quantity_receive);
  ...
  return h('div', {
    ...
    style: { position: 'relative', width: '100%', maxWidth: 1760, minWidth: 0, margin: '0 auto', padding: '12px 16px 16px', background: '#fbfcfe', boxSizing: 'border-box', whiteSpace: 'normal' },
  },
    ...
    simOn && (changedKeys.length || draft?.pending) ? h('div', { style: { margin: '6px 0 8px', display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', fontSize: 12, fontWeight: 700, color: '#4b2fb0', background: '#f6f3ff', border: '1px dashed #8b6cf0', borderRadius: 8, padding: '6px 11px' } },
      ...
      h('span', { style: { marginLeft: 'auto', fontWeight: 400, color: '#8a7fc0', fontSize: 11 } }, '拖圆点：上下 = 数量（1 格 = 1 天库存）· 左右 = 时间（按周）· 松开后确认；安全区间由服务端工作流重算')) : null,
    h('div', {
      ...
      h('svg', { viewBox: `0 0 ${W} ${H}`, shapeRendering: 'geometricPrecision', style: { position: 'absolute', inset: 0, width: '100%', height: '100%', display: 'block' } },
        ...
        ...visibleNodes.flatMap((node) => {
          ...
          return [
            h('circle', { key: `node-dot-${node.index}`, cx: node.x, cy: node.y, r: 6.5, fill: modified ? '#f6f3ff' : '#fff', stroke: locked ? '#b06a00' : '#5b7cfa', strokeWidth: 2.3, strokeDasharray: locked ? undefined : '3 2', pointerEvents: 'none' }),
            h('text', { key: `node-qty-${node.index}`, x: node.x, y: node.y + (above ? -28 : 21), fill: modified ? '#5b3fc4' : '#1f2329', fontSize: 12.5, fontWeight: 800, ...textStyle }, `W${node.index + 1} · ${fmt(node.qty)} 台${modified ? '*' : ''}`),
            h('text', { key: `node-date-${node.index}`, x: node.x, y: node.y + (above ? -14 : 35), fill: modified ? '#5b3fc4' : '#5a6169', fontSize: 12, fontWeight: 600, ...textStyle }, `到货 ${shortDate(node.arrival)}${locked ? ' 🔒' : ''}`),
          ];
        })),
      ...visibleNodes.map((node) => {
        return h('button', {
          key: `node-${node.index}`, type: 'button', 'aria-label': `W${node.index + 1} · ${fmt(node.qty)} 台 · 到货 ${shortDate(node.arrival)}`, onPointerDown: (event) => startDrag(event, node),
      ...
      hover ? h(React.Fragment, null,
        ...
        h('div', { style: { position: 'absolute', ...(hover.x / W > 0.76 ? { right: 12 } : { left: `calc(${xPercent(hover.x)} + 9px)` }), top: 32, zIndex: 6, width: 248, maxWidth: 'calc(100% - 24px)', boxSizing: 'border-box', whiteSpace: 'normal', overflowWrap: 'anywhere', wordBreak: 'break-word', borderRadius: 6, padding: '8px 10px', background: 'rgba(31,35,41,.92)', color: '#fff', fontSize: 12, lineHeight: 1.55, pointerEvents: 'none', boxShadow: '0 5px 16px rgba(0,0,0,.18)' } },
          h('div', null, `${hover.date}　新算法 ${fmt(hover.sys, 1)} 天`),
          h('div', { style: { color: '#ffd479' } }, `现有水位 ${fmt(hover.existing, 1)} 天`),
          hover.daily ? h('div', { style: { color: '#cbd6ff' } }, `v2 库存 ${fmt(hover.daily.v2_inventory)} · 在途 ${fmt(hover.daily.v2_on_the_way)} · 当日入库 ${fmt(hover.daily.v2_add)}`) : null)) : null,
    ...
    h('div', { style: { margin: '6px 0 0 40px', display: 'flex', gap: 14, flexWrap: 'wrap', fontSize: 11, color: '#5a6169' } },
      h('span', null, h('i', { style: { display: 'inline-block', width: 18, borderTop: '2px solid #5b7cfa', marginRight: 5, verticalAlign: 'middle' } }), '新算法安全天数（实际历史 → v2 未来）'),
      h('span', null, h('i', { style: { display: 'inline-block', width: 18, borderTop: '2px solid #e0a53a', marginRight: 5, verticalAlign: 'middle' } }), '现有水位安全天数'),
...
function ShipmentEvolutionBlockV19() {
  ...
  useEffect(() => {
    ...
    const unsubscribers = routers.map((router) => router.subscribe(() => {
      ...
      setSelectedSale(sale); setParams({ ...next, sale, shop: next.shop || TOTAL_SHOP }); setChanges({}); setConfirmedRows({});
    }));
    return () => unsubscribers.forEach((unsubscribe) => unsubscribe?.());
  }, [catalogReady, products]);
  ...
  const loadData = useCallback(async () => {
    if (!catalogReady) return;
    const requestId = ++requestSequence.current;
    if (!scopedProducts.length) { setDailyRows([]); setTotalRows([]); setShipments([]); setWaterRows([]); setModelLevels([]); setLogisticsLeads([]); setChanges({}); setShops([]); setLoading(false); return; }
    setLoading(true); setError('');
    try {
      const activeShop = params.shop || TOTAL_SHOP;
      const dailyPromise = requestDailySales(scopedProducts, activeShop);
      const directoryShipmentPromise = requestShipments(shopScopeProducts, TOTAL_SHOP);
      const [shopData, directoryShipments, dailyData, totalData, shipmentData, waterData, levelData, logisticsData] = await Promise.all([
        requestShops(shopScopeProducts), directoryShipmentPromise, dailyPromise,
        activeShop === TOTAL_SHOP ? dailyPromise : requestDailySales(scopedProducts, TOTAL_SHOP),
        activeShop === TOTAL_SHOP && shopScopeSignature === scopeSignature ? directoryShipmentPromise : requestShipments(scopedProducts, activeShop),
      ...
      [...shopData, ...directoryShipments].forEach((item) => {
        if (!item.shop || item.shop === TOTAL_SHOP) return;
        if (!shopProducts.has(item.shop)) shopProducts.set(item.shop, new Set());
        shopProducts.get(item.shop).add(rowProductKey(item));
      });
      const nextShops = Array.from(shopProducts, ([name, keys]) => ({ name, productCount: keys.size, productKeys: Array.from(keys) }))
        .sort((a, b) => String(a.name).localeCompare(String(b.name), 'zh-CN'));
      setShops(nextShops);
      if (activeShop !== TOTAL_SHOP && !shopProducts.has(activeShop)) {
        setParams((current) => ({ ...current, shop: TOTAL_SHOP }));
        replaceShopParam(TOTAL_SHOP, Boolean(params.asin));
      }
      setDailyRows(dailyData); setTotalRows(totalData); setShipments(shipmentData); setWaterRows(waterData); setModelLevels(levelData); setLogisticsLeads(logisticsData);
      setChanges(buildWorkflowChanges(scopedProducts, shipmentData, planChangeData, changeLogData));
    } catch (requestError) {
      if (requestId === requestSequence.current) { setError(requestError?.message || String(requestError)); setDailyRows([]); setTotalRows([]); setShipments([]); setWaterRows([]); setModelLevels([]); setLogisticsLeads([]); setChanges({}); }
    } finally { if (requestId === requestSequence.current) setLoading(false); }
  }, [catalogReady, scopeSignature, shopScopeSignature, params.shop, refreshSeed]);
  useEffect(() => { loadData(); }, [loadData]);
  const productViews = useMemo(() => scopedProducts.map((product) => { const key = productKey(product); const dataKey = rowProductKey(product); const matches = (item) => rowProductKey(item) === dataKey; const productDaily = dailyRows.filter(matches); const productTotal = totalRows.filter(matches); const productPlans = shipments.filter(matches); const waterRow = waterRows.find(matches) || null; const current = productDaily.find((item) => dateText(item.date) === todayText()) || latestOnOrBefore(productDaily, todayText()); const total = productTotal.find((item) => dateText(item.date) === todayText()) || latestOnOrBefore(productTotal, todayText()); return { key, dataKey, product, dailyRows: productDaily, totalRow: total, waterRow, summaryRow: current, ratio: ratioInfo(waterRow), levelName: modelLevels.find((item) => item.country === product.country && item.model === product.model)?.level_name || '', weeks: buildWeeks(productPlans), netWeeks: buildWeeks(productPlans), noShopData: params.shop !== TOTAL_SHOP && !current }; }).filter((row) => params.shop === TOTAL_SHOP || shopProductKeys.get(params.shop)?.has(row.dataKey)), [scopedProducts, dailyRows, totalRows, shipments, waterRows, modelLevels, params.shop, shopProductKeys]);
  ...
  function changeShop(shop) { setParams((current) => ({ ...current, shop })); setChanges({}); setConfirmedRows({}); replaceShopParam(shop, Boolean(params.asin)); }
  ...
  return h('div', { style: { width: '100%', minWidth: 0, margin: 0, padding: 22, background: '#eef1f5', color: '#1f2329', fontFamily: '-apple-system,"PingFang SC","Microsoft YaHei",sans-serif', fontSize: 13.5, lineHeight: 1.6, boxSizing: 'border-box', WebkitFontSmoothing: 'antialiased', fontVariantNumeric: 'tabular-nums' } },
    ...
    h('div', { style: { background: '#fff', border: '1px solid #e6e9ee', borderRadius: 9, boxShadow: '0 1px 8px rgba(0,0,0,0.05)', padding: '16px 18px' } },
      ...
      catalogLoading || loading ? h('div', { style: { minHeight: 360, display: 'flex', alignItems: 'center', justifyContent: 'center' } }, h(Spin, { size: 'large', tip: '正在读取 NocoBase 实际数据...' })) : visibleRows.length ? h(V19Table, { rows: visibleRows, allScopeRows: productViews, changes, confirmedRows, role, orderWeek, poApproved, logisticsLeads, onEdit: openEdit, onOpenDetail: (row, weekIndex) => { setAuditNote(''); setDetail({ row, weekIndex }); } }) : h(Empty, { description: mine ? '当前角色没有待处理商品' : '当前筛选范围没有商品' }),
