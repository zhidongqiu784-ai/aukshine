...
function ShipmentEvolutionBlockV19() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const [dailyData, shipmentData, waterData, levelData, logisticsData] = await Promise.all([
        dailyPromise, shipmentPromise,
      ...
      if (requestId !== requestSequence.current) return;
      setShops([]);
      setDailyRows(dailyData); setTotalRows(dailyData); setShipments(shipmentData); setWaterRows(waterData); setModelLevels(levelData); setLogisticsLeads(logisticsData);
