      ...
      h('svg', { viewBox: `0 0 ${W} ${H}`, shapeRendering: 'geometricPrecision', style: { position: 'absolute', inset: 0, width: '100%', height: '100%', display: 'block' } },
        ...
        ...visibleNodes.flatMap((node) => {
          ...
          return [
            ...
            hasQty ? h('text', { key: `node-qty-${node.index}`, x: node.x, y: labelY, fill: modified ? '#5b3fc4' : locked ? '#8a5a00' : '#1f2329', fontSize: modified ? 12.6 : 12, fontWeight: 800, ...textStyle }, `W${node.index + 1} · ${fmt(node.totalQty)}${modified ? '*' : ''}`) : null,
          ];
        })),
