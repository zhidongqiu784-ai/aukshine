...
formula_sequence_calc AS (
    SELECT
        ...
        @seq_service_demand := CASE
            WHEN fc.cycle_phase = 'ORDER_WEEK' THEN COALESCE(fc.second_week_demand, 0)
            ELSE COALESCE(fc.first_week_demand, 0)
        END AS sequential_service_demand,
        @seq_raw_inventory := CASE
            WHEN fc.cycle_phase = 'ORDER_WEEK' THEN fc.inventory_at_second_service_start
            ELSE fc.inventory_at_first_service_start
        END AS sequential_raw_inventory_at_service_start,
        ...
        @seq_candidate := CASE
            WHEN fc.cycle_phase = 'ORDER_WEEK'
             AND fc.second_week_demand_days <> 7 THEN NULL
            WHEN fc.cycle_phase <> 'ORDER_WEEK'
             AND fc.first_week_demand_days <> 7 THEN NULL
...
candidate_rows AS (
    SELECT
        ...
        CASE
            ...
            WHEN gc.cycle_phase = 'ORDER_WEEK'
             AND gc.inventory_at_second_service_start IS NULL
                THEN 'START_INVENTORY_MISSING'
            WHEN gc.cycle_phase <> 'ORDER_WEEK'
             AND gc.inventory_at_first_service_start IS NULL
                THEN 'START_INVENTORY_MISSING'
...
SELECT
    ...
    cr.asin AS `ASIN`,
    cr.shop AS `店铺`,
    CONCAT('W', cr.week_no) AS `周序号`,
    CASE
        WHEN cr.cycle_phase = 'ORDER_WEEK' THEN '下单周-A2'
        ELSE '非下单周-A1'
    END AS `节点类型`,
    CASE
        WHEN cr.generation_status = 'READY'
         AND (
    ...
    END AS `是否会写入`,
    CASE
        WHEN cr.generation_status <> 'READY' THEN CASE cr.generation_status
        ...
        END
        WHEN cr.cycle_phase = 'ORDER_WEEK'
         AND rn.region_net_status <> 'READY' THEN CASE rn.region_net_status
            WHEN 'TODAY_TOTAL_ROW_MISSING' THEN '当天合计销量记录缺失'
            WHEN 'SHOP_BATCH_WEEK_NOT_ALIGNED' THEN '同区域店铺下单批周次不一致'
            WHEN 'SHOP_CANDIDATE_NOT_ALL_READY' THEN '同区域仍有候选计划未准备完成'
            ...
            WHEN 'RPA_ASSIGNMENT_NOT_MAX_WEIGHTED_SALES' THEN '未交货订单分配校验失败：未分配给加权销量最高ASIN'
            WHEN 'RPA_ASSIGNMENT_DUPLICATE_TODAY_TOTAL_ROW' THEN '未交货订单分配校验失败：当天合计记录重复'
            ELSE CONCAT('未识别区域状态：', COALESCE(rn.region_net_status, '空值'))
        END
        ELSE '可生成'
    END AS `阻断原因`,
    cr.candidate_ship_date AS `发货日期`,
    cr.candidate_add_date AS `预计入库日`,
    cr.candidate_season AS `季节`,
    cr.channel AS `物流渠道`,
    cr.logistics_days AS `物流天数`,
    cr.candidate_warehouse_days AS `入仓天数`,
    cr.sequential_candidate_number AS `生成数量`,
    ROUND(cr.sequential_service_demand, 2) AS `服务周需求`,
    ROUND(cr.sequential_raw_inventory_at_service_start, 2) AS `服务期原始库存`,
    ROUND(cr.seq_previous_candidate, 2) AS `前序已生成候选量`,
    ROUND(cr.sequential_inventory_at_service_start, 2) AS `服务期有效库存`,
    ROUND(
        cr.sequential_service_demand - cr.sequential_inventory_at_service_start,
        2
    ) AS `需求减有效库存`,
    cr.candidate_number AS `原始公式数量`,
    CONCAT(
        DATE_FORMAT(cr.first_service_start_date, '%Y-%m-%d'),
        ' ~ ',
        DATE_FORMAT(cr.first_service_end_date, '%Y-%m-%d')
    ) AS `第一服务周`,
    CASE
        WHEN cr.second_service_start_date IS NULL THEN ''
        ELSE CONCAT(
            DATE_FORMAT(cr.second_service_start_date, '%Y-%m-%d'),
            ' ~ ',
            DATE_FORMAT(cr.second_service_end_date, '%Y-%m-%d')
        )
    END AS `第二服务周`,
    rn.quantity_receive AS `区域未交货订单量`,
    rn.earlier_committed_number AS `区域更早承诺量`,
    rn.region_a1_number AS `区域A1量`,
    rn.region_a2_number AS `区域A2量`,
    rn.net_order_number AS `区域净下单量`,
