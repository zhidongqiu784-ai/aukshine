...
function V19TrendChart({ row, changes, role, poApproved, orderWeek, channelOptions, onApply, onSandbox }) {
  ...
  const actualSupplyByDate = new Map();
  (row.realSupplyRows || []).forEach((item) => {
    const key = dateText(item.expected_storage_time);
    if (!key) return;
    actualSupplyByDate.set(key, numberValue(actualSupplyByDate.get(key)) + numberValue(item.remaining));
  });
  ...
  const actualMarkers = Array.from(actualSupplyByDate.entries()).map(([date, qty]) => {
    const pointIndex = dates.indexOf(date);
    if (pointIndex < 0 || qty <= 0) return null;
    return { date, qty, pointIndex, x: px(pointIndex), y: py(nearestValue(sysValues, pointIndex, 0)) };
  }).filter(Boolean);
  ...
  const hover = dragging || hoveredBatch || hoverIndex == null ? null : {
    ...
    actualSupply: numberValue(actualSupplyByDate.get(dates[hoverIndex])),
  };
  ...
  return h('div', {
    ...
    h('div', {
      ...
      h('svg', { viewBox: `0 0 ${W} ${H}`, shapeRendering: 'geometricPrecision', style: { position: 'absolute', inset: 0, width: '100%', height: '100%', display: 'block' } },
        ...
        ...actualMarkers.map((item) => h('g', { key: `actual-${item.date}`, pointerEvents: 'none' },
          h('rect', { x: item.x - 5, y: item.y - 5, width: 10, height: 10, rx: 2, fill: '#fffaf1', stroke: '#b06a00', strokeWidth: 2, transform: `rotate(45 ${item.x} ${item.y})` }),
          h('text', { x: item.x, y: item.y + (item.y > T + 42 ? -18 : 25), textAnchor: 'middle', paintOrder: 'stroke', stroke: '#fff', strokeWidth: 3, strokeLinejoin: 'round', fill: '#8a5a00', fontSize: 11.5, fontWeight: 800 }, `实际 ${fmt(item.qty)} 🔒`))),
        ...
        ...visibleNodes.flatMap((node) => {
          ...
          return [
            ...
            h('text', { key: `node-qty-${node.index}`, x: node.x, y: node.y + (above ? -28 : 21), fill: modified ? '#5b3fc4' : '#1f2329', fontSize: 12.5, fontWeight: 800, ...textStyle }, `W${node.index + 1} · 建议 ${fmt(node.qty)} 台${modified ? '*' : ''}`),
      ...
      hover ? h(React.Fragment, null,
        ...
        h('div', { style: { position: 'absolute', ...(hover.x / W > 0.76 ? { right: 12 } : { left: `calc(${xPercent(hover.x)} + 9px)` }), top: 32, zIndex: 6, width: 248, maxWidth: 'calc(100% - 24px)', boxSizing: 'border-box', whiteSpace: 'normal', overflowWrap: 'anywhere', wordBreak: 'break-word', borderRadius: 6, padding: '8px 10px', background: 'rgba(31,35,41,.92)', color: '#fff', fontSize: 12, lineHeight: 1.55, pointerEvents: 'none', boxShadow: '0 5px 16px rgba(0,0,0,.18)' } },
          ...
          hover.actualSupply ? h('div', { style: { color: '#ffd479' } }, `真实在途入库 ${fmt(hover.actualSupply)} 台（锁定，不可拖）`) : null,
      ...
      hoveredBatch ? h('div', { style: { position: 'absolute', ...(hoveredBatch.x / W > 0.76 ? { right: 12 } : { left: `calc(${xPercent(hoveredBatch.x)} + 12px)` }), ...(hoveredBatch.y / H > 0.52 ? { bottom: `calc(${Math.max(5, 100 - hoveredBatch.y / H * 100)}% + 14px)` } : { top: `calc(${Math.max(5, hoveredBatch.y / H * 100)}% + 14px)` }), zIndex: 6, width: 300, maxWidth: 'calc(100% - 24px)', maxHeight: 'calc(100% - 20px)', overflowY: 'auto', boxSizing: 'border-box', whiteSpace: 'normal', overflowWrap: 'anywhere', wordBreak: 'break-word', padding: '9px 11px', borderRadius: 7, background: 'rgba(31,35,41,0.94)', color: '#fff', fontSize: 12, lineHeight: 1.55, pointerEvents: 'none', boxShadow: '0 6px 22px rgba(0,0,0,.28)' } },
        ...
        h('div', null, `建议数量 ${fmt(hoveredBatch.qty)} 台 · 渠道 ${hoveredBatch.mod.channel || hoveredBatch.channel}`),
    ...
    h('div', { style: { margin: '6px 0 0 40px', display: 'flex', gap: 14, flexWrap: 'wrap', fontSize: 11, color: '#5a6169' } },
      ...
      h('span', null, h('i', { style: { display: 'inline-block', width: 18, borderTop: '2px dashed #c0392b', marginRight: 5, verticalAlign: 'middle' } }), '上下限（站点×淡旺季）'),
        h('span', null, '◌ 建议节点（W1–W7 · 点=详情/改渠道 · 拖=改量/改期 · 橙=W1–W2 守工厂节奏）'),
      actualMarkers.length ? h('span', { style: { color: '#8a5a00' } }, '◇ 真实在途入库（expected_inventory · 锁定）') : null,
  ...
  return h('div', null,
    h('div', { style: { overflowX: 'auto', border: '1px solid #e3e7ee', borderRadius: 8 } }, h('table', { style: { borderCollapse: 'separate', borderSpacing: 0, fontSize: 14, whiteSpace: 'nowrap', minWidth: w12Open ? 1680 : 1580, width: '100%' } },
      ...
      h('tbody', null,
        ...rows.flatMap((row, rowIndex) => {
          ...
          const cells = shownIndices.map((index) => {
            if (index === 'fold') {
              ...
              return h('td', { key: 'fold', onClick: () => setW12Open(true), style: { background: '#ededf0', color: '#6a727d', fontSize: 11, cursor: 'pointer', minWidth: 84, borderBottom: border, borderRight: border, textAlign: 'center', fontWeight: 700 } }, weekCellBody(actual + suggest, actual, suggest));
            }
            const week = row.weeks[index]; const key = v19ChangeKey(row.key, index); const change = changes[key]; const value = v19WeekValue(row, index, changes); const actual = v19ActualWeekValue(row, index); const sandbox = sandboxes[row.key]?.[index]; const displaySuggest = sandbox?.qty == null ? value : numberValue(sandbox.qty); const displayValue = displaySuggest + actual; const status = change?.status || 'ok'; const statusStyle = V19_STATUS_STYLE[status]; const newAlgorithm = week.newQty > 0; const actualLocked = actual > 0;
        ...
        h('tr', null, h('td', { colSpan: 9, style: { background: '#eef3fa', fontWeight: 800, borderTop: '2px solid #c3cfe0', textAlign: 'right', padding: '8px 12px', color: '#3a4763' } }, 'Σ 每周到货合计 ', h('span', { style: { fontSize: 11, color: '#1a5fb4', background: '#e7f0fd', border: '1px solid #b9d4f5', borderRadius: 6, padding: '1px 9px', marginLeft: 6 } }, `${role === 'sale' ? '我的 ASIN' : role === 'lead' ? '本部门' : '公司'} · ${allScopeRows.length} ASIN`)), ...shownIndices.map((index) => {
          ...
          return h('td', { key: index, style: { background: '#eef3fa', fontWeight: 800, borderTop: '2px solid #c3cfe0', textAlign: 'center', padding: '8px 6px' } }, weekCellBody(actual + suggest, actual, suggest));
