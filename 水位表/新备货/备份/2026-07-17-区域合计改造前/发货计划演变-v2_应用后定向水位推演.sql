...
--       ASIN + country + shop，并同步重算该 ASIN + country 的“合计”行。
--
-- 输入绑定（必须在创建 NocoBase 工作流 SQL 节点时替换下面三个 NULL）：
-- - 右侧必须绑定服务端已校验的申请记录字段，不能直接信任前端值。
-- - 三个输入必须与本文件其他语句处于同一个 SQL 节点、同一个数据库连接。
-- - 未绑定时 CHECK 防护会主动失败，不会更新 daily_sales。
SET @v2_target_asin = CAST(NULL AS CHAR);
SET @v2_target_country = CAST(NULL AS CHAR);
SET @v2_target_shop = CAST(NULL AS CHAR);
...
SET @v2_target_shop = NULLIF(TRIM(@v2_target_shop), '');
...
DROP TEMPORARY TABLE IF EXISTS temp_v2_target_total_stage;
...
VALUES (
    IF(
        ...
        AND @v2_target_shop IS NOT NULL
...
-- 定向合计依赖其他分店最近一次完整 v2 结果；未初始化时主动失败，等待全量节点先跑。
INSERT INTO temp_v2_target_guard (guard_value)
SELECT IF(COUNT(*) = 0, 1, 0)
FROM daily_sales AS other_shop
WHERE other_shop.asin = @v2_target_asin
  AND other_shop.country = @v2_target_country
  AND other_shop.shop <> '合计'
  AND other_shop.shop <> @v2_target_shop
  AND other_shop.`date` >= CURDATE()
  AND (
        other_shop.v2_inventory IS NULL
        OR other_shop.v2_days_for_sale IS NULL
        OR other_shop.v2_on_the_way IS NULL
        OR other_shop.v2_calculated_at IS NULL
      );

-- 目标区域每一天都必须同时存在目标店铺新结果和“合计”行。
INSERT INTO temp_v2_target_guard (guard_value)
SELECT IF(COUNT(*) = 0, 1, 0)
FROM temp_v2_target_branch_stage AS branch
LEFT JOIN daily_sales AS total
    ON total.asin = branch.asin
   AND total.country = branch.country
   AND total.shop = '合计'
   AND total.`date` = branch.`date`
WHERE total.shop_country_asin_date IS NULL;

CREATE TEMPORARY TABLE temp_v2_target_total_stage AS
SELECT
    total.asin,
    total.country,
    total.shop,
    ...
    NULLIF(
        CAST(
            SUM(
                COALESCE(
                    CASE
                        WHEN member.shop = @v2_target_shop THEN target_branch.v2_add
                        ELSE member.v2_add
                    END,
                    0
                )
    ...
    CAST(
        SUM(
            CASE
                WHEN member.shop = @v2_target_shop THEN target_branch.v2_inventory
                ELSE member.v2_inventory
            END
        ) AS SIGNED
    ) AS v2_inventory,
    CAST(
    ...
    CAST(
        SUM(
            CASE
                WHEN member.shop = @v2_target_shop THEN target_branch.v2_on_the_way
                ELSE member.v2_on_the_way
            END
        ) AS SIGNED
    ) AS v2_on_the_way,
    @v2_calculated_at AS v2_calculated_at
FROM daily_sales AS total
INNER JOIN daily_sales AS member
    ON member.asin = total.asin
   AND member.country = total.country
   AND member.`date` = total.`date`
   AND member.shop <> '合计'
LEFT JOIN temp_v2_target_branch_stage AS target_branch
    ON target_branch.asin = member.asin
   AND target_branch.country = member.country
   AND target_branch.shop = member.shop
   AND target_branch.`date` = member.`date`
WHERE total.asin = @v2_target_asin
  AND total.country = @v2_target_country
  AND total.shop = '合计'
  AND total.`date` >= CURDATE()
GROUP BY
    total.shop_country_asin_date,
    total.asin,
    total.country,
    total.shop,
    total.`date`,
...
ALTER TABLE temp_v2_target_total_stage
    ADD UNIQUE INDEX idx_v2_target_total_stage_key (asin, country, shop, `date`);

INSERT INTO temp_v2_target_guard (guard_value)
SELECT IF(
    (SELECT COUNT(*) FROM temp_v2_target_total_stage)
    = (SELECT COUNT(*) FROM temp_v2_target_branch_stage),
    1,
    0
);
...
UNION ALL
SELECT * FROM temp_v2_target_total_stage;

ALTER TABLE temp_v2_target_write_stage
    ADD UNIQUE INDEX idx_v2_target_write_stage_key (asin, country, shop, `date`);

SET @v2_target_branch_rows = (SELECT COUNT(*) FROM temp_v2_target_branch_stage);
SET @v2_target_total_rows = (SELECT COUNT(*) FROM temp_v2_target_total_stage);
...
WHERE target.`date` >= CURDATE()
  ...
  AND target.shop IN (@v2_target_shop, '合计')
  AND (
