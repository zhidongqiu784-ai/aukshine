...
DROP TEMPORARY TABLE IF EXISTS temp_v2_total_stage;
...
-- 每个可推演分店日期必须有“合计”行。
INSERT INTO temp_v2_projection_guard (guard_name, guard_value)
SELECT 'BRANCH_HAS_TOTAL_ROW', IF(COUNT(*) = 0, 1, 0)
FROM (
    SELECT branch.asin, branch.country, branch.`date`
    FROM temp_v2_branch_stage AS branch
    ...
    GROUP BY branch.asin, branch.country, branch.`date`
) AS branch_without_total;

CREATE TEMPORARY TABLE temp_v2_total_stage AS
SELECT
    total.asin,
    total.country,
    total.shop,
    total.`date`,
    NULLIF(CAST(SUM(COALESCE(branch.v2_add, 0)) AS SIGNED), 0) AS v2_add,
    CAST(SUM(branch.v2_inventory) AS SIGNED) AS v2_inventory,
    CAST(
        CASE
            WHEN SUM(branch.v2_inventory) <= 0 THEN 0
            WHEN total.weighted_sales > 0
            THEN FLOOR(SUM(branch.v2_inventory) / total.weighted_sales)
            ELSE 0
        END AS SIGNED
    ) AS v2_days_for_sale,
    CAST(SUM(branch.v2_on_the_way) AS SIGNED) AS v2_on_the_way,
    @v2_calculated_at AS v2_calculated_at
FROM daily_sales AS total
INNER JOIN temp_v2_branch_stage AS branch
...
ALTER TABLE temp_v2_total_stage
    ADD INDEX idx_v2_total_stage_key (asin, country, shop, `date`);

INSERT INTO temp_v2_projection_guard (guard_name, guard_value)
SELECT 'TOTAL_STAGE_COVERS_BRANCH_GROUPS', IF(
    (SELECT COUNT(*) FROM temp_v2_total_stage)
    = (
        SELECT COUNT(*)
        FROM (
...
SET @v2_preview_branch_rows = (SELECT COUNT(*) FROM temp_v2_branch_stage);
SET @v2_preview_total_rows = (SELECT COUNT(*) FROM temp_v2_total_stage);
